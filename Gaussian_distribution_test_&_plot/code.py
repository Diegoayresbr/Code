...
def normality_test(test_data):
    ...
    fig, (ax1, ax2) = plt.subplots(nrows=1, ncols=2, figsize=(13, 6), dpi=80, sharex=False)
    ...
    sns.kdeplot(test_data
                , color='darkblue'
                 , label='KDE - kernel density estimation'
                 , ax=ax1
                 )
    ...
    ax1.plot(x, stats.norm.pdf(x, avg, sigma)
             , color='yellow', alpha=0.8, linewidth=2, label='normal distribution')
    ax1.legend(loc='best', fontsize='x-small', title=None, bbox_to_anchor=None).set_draggable(True)
