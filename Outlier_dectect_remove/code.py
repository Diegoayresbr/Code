import pandas as pd
import numpy as np
from sklearn.covariance import EllipticEnvelope # For multivariate df with Normal Distribution
...
def outlier_detector(df, percentage=0.01):
    ...
    predict_x = model_detection.fit_predict(df)  # prediction [1 or -1] # -1 are outliers
    ...
    df_check['%_of_Total'] = df_check.iloc[:, 0].apply(lambda x: x / sum_total)  # .astype(str) + '%'
    print(df_check)
